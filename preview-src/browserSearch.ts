...
/*
 * Copied from microsoft/vscode:
 * src/vs/workbench/contrib/browserView/common/browserSearch.ts
 * (at 1.134.0-1325-gaa7291eba7d). Only change: the `localize` calls were
 * inlined as plain strings, since this module is bundled into the webview
 * where vscode.l10n is not available, and `enum BrowserSearchEngineId` became
 * a const object + type alias so the file stays erasable TypeScript (Node's
 * native type stripping runs the tests, and it rejects `enum`).
 * One addition: the exported `hasKnownScheme` helper (marked at its definition).
 * Keep in sync deliberately; the unit tests in browserSearch.test.ts are the
 * specification.
 */
 ...
 */
export const BrowserSearchEngineId = {
	Bing: 'bing',
	Google: 'google',
	Yahoo: 'yahoo',
...
export type BrowserSearchEngineId =
	typeof BrowserSearchEngineId[keyof typeof BrowserSearchEngineId];
...
/**
 * Whether `input` starts with a scheme we actually recognize. Added on top of
 * the upstream file: callers that turn a `'url'` classification into something
 * navigable need to know whether to prepend a scheme, and a bare syntactic
 * check gets `localhost:3000` wrong (see {@link ALL_KNOWN_SCHEMES}).
 */
export function hasKnownScheme(input: string): boolean {
	const match = SCHEME_REGEX.exec(input);
	return match !== null && ALL_KNOWN_SCHEMES.has(match[1].toLowerCase());
}
